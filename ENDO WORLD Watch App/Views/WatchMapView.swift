...
// Spec: outer pulse 44pt, strokeBorder 12pt, scale 1.0→1.3, fade, 1.6s repeat.
// Mid ring 22pt strokeBorder 2pt. Core 10pt. White center 4pt.
...
struct PersonalPinView: View {
    ...
    var body: some View {
        ZStack {
            Circle()
                .strokeBorder(zone.color.opacity(0.35), lineWidth: 12)
...
// Spec: outer breathing ring strokeBorder 5pt, scale 1.0→1.8, fade, repeat.
// Pulse speed by zone: hostile 1.2s, moderate 1.8s, supportive 2.4s.
// Core dot: 5pt base + (compositeScore/100 × 4pt).
...
struct NeighborhoodPinView: View {
    ...
    private var coreSize: CGFloat {
        5 + 4 * CGFloat(pin.compositeScore) / 100
    }

    var body: some View {
        ZStack {
            Circle()
                .strokeBorder(pin.zone.color.opacity(0.5), lineWidth: 5)
                .frame(width: 16, height: 16)
                .scaleEffect(breathing ? 1.8 : 1.0)
                .opacity(breathing ? 0.0 : 0.7)
                .animation(
                    .easeOut(duration: pin.zone.pulseDuration).repeatForever(autoreverses: false),
                    value: breathing
                )
            Circle()
