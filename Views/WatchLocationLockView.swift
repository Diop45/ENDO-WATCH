...
struct WatchLocationLockView: View {
    ...
    private func globeLayer(geo: GeometryProxy) -> some View {
        ...
        ZStack {
            ...
            // Active scan ring — dashed ring rotating around user dot position
            Circle()
                .trim(from: 0, to: 0.25)
                .stroke(
                ...
                .offset(x: W * 0.5 - W * 0.5 - R * 0.05, y: -(R - R * 0.07))
                .animation(.linear(duration: 3).repeatForever(autoreverses: false), value: scanAngle)

            // User location dot — pulsing green
            userLocationDot
                .offset(x: -R * 0.05, y: -(R - R * 0.07))
        }
    ...
    private var topChrome: some View {
        ZStack {
            // Satellite / GPS icon — top center, floating above globe
