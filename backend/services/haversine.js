'use strict';

const EARTH_KM = 6371;

/** Returns distance in kilometres between two lat/lon pairs. */
function haversineKm(lat1, lon1, lat2, lon2) {
    const toRad = (d) => d * Math.PI / 180;
    const dLat  = toRad(lat2 - lat1);
    const dLon  = toRad(lon2 - lon1);
    const a =
        Math.sin(dLat / 2) ** 2 +
        Math.cos(toRad(lat1)) * Math.cos(toRad(lat2)) * Math.sin(dLon / 2) ** 2;
    return EARTH_KM * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
}

/** Returns distance in miles. */
function haversineMiles(lat1, lon1, lat2, lon2) {
    return haversineKm(lat1, lon1, lat2, lon2) * 0.621371;
}

/**
 * Returns bounding box {minLat, maxLat, minLon, maxLon} for a radius in miles.
 * Used for a fast SQL pre-filter before the exact haversine check.
 */
function boundingBox(lat, lon, radiusMiles) {
    const km     = radiusMiles * 1.60934;
    const dLat   = km / EARTH_KM;
    const dLon   = km / (EARTH_KM * Math.cos(lat * Math.PI / 180));
    return {
        minLat: lat - dLat * (180 / Math.PI),
        maxLat: lat + dLat * (180 / Math.PI),
        minLon: lon - dLon * (180 / Math.PI),
        maxLon: lon + dLon * (180 / Math.PI),
    };
}

module.exports = { haversineKm, haversineMiles, boundingBox };
